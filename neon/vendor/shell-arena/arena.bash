# Shell Arena v2 / Neon Link. Source AFTER the existing Atuin/Starship setup.
# No keybindings, DEBUG trap, PROMPT_COMMAND or Neovim configuration changes.
[[ $- == *i* && -n ${BASH_VERSION:-} ]] || return 0
[[ -z ${_ARENA_LOADED:-} ]] || return 0
if [[ -z ${bash_preexec_imported:-} ]] && ! declare -F __bp_precmd_invoke_cmd >/dev/null; then
    printf 'Shell Arena: load Atuin before arena.bash, then open a fresh Bash terminal.\n' >&2
    return 0
fi
_ARENA_DIR="${BASH_SOURCE[0]%/*}"
[[ $_ARENA_DIR != "${BASH_SOURCE[0]}" ]] || _ARENA_DIR=.
_ARENA_DIR=$(CDPATH= cd -- "$_ARENA_DIR" && pwd -P) || return 0
_ARENA_LOADED=1
...
_arena_read_state() {
    ...
    read -r xp wins muted enabled hud vibe volume <<< "$line"
...
_arena_audio_stop() {
    ...
    if [[ -n $_ARENA_AUDIO_RUN ]]; then
        ...
        [[ ! -p $_ARENA_AUDIO_RUN/events ]] || unlink -- "$_ARENA_AUDIO_RUN/events"
...
_arena_neon_draw() {
    ...
    local meter='' i node status=$_ARENA_LAST line detail place=${PWD##*/}
    [[ $width =~ ^[0-9]{1,5}$ ]] || width=80
    width=$((10#$width))
    ((width >= 2)) || return 0
    width=$((width - 1))
    ...
    if [[ -z ${NO_COLOR+x} && ${TERM:-} != dumb ]]; then
        ...
        [[ $status != RETRY* ]] || accent=$'\e[38;2;255;154;112m'
        [[ $status != RECOVERED* && $status != 'LEVEL UP'* ]] || accent=$'\e[38;2;255;114;213m'
    fi
    for ((i=0; i<6; i++)); do
        if ((i < progress / 50)); then meter+='━'; else meter+='─'; fi
    done
    case $((_ARENA_OPS % 4)) in 0) node=◇;; 1) node=◈;; 2) node=◆;; 3) node=◈;; esac
    case $status in CLEAR*) status="ACK${status#CLEAR}";; READY) status='LINK READY';; esac
    _arena_stage_count
    ...
    if ((width >= 69)); then
        ...
        line="$node ARENA L$level / $status / $_ARENA_OPS OPS"
        printf '%s%s%s\n' "$ink" "${line:0:width}" "$reset" >&2
    fi
    return 0
}
...
arena() {
    ...
    case $action in
        ...
        mute|unmute)
            ...
            if [[ $value == 1 ]]; then _arena_audio_stop; else _arena_audio_start; fi
        ...
        volume)
            ...
            _arena_audio_start
            printf 'Arena volume: %s%% of source level. Windows volume also applies.\n' "$_ARENA_VOLUME"
            ;;
        soundtest)
            _arena_audio_stop
            _arena_profile
        ...
        help)
            printf '%s\n' \
                ...
                'arena volume 60          Save volume, 0-100; Windows volume also applies' \
...
preexec_functions+=(_arena_preexec)
precmd_functions+=(_arena_precmd)
complete -W 'quest predict check recall finish focus status abandon mute unmute hud vibe volume off on soundtest help' arena
return 0
